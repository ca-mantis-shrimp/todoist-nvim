(let [M {}]
  (tset M :url "https://api.todoist.com/sync/v9/sync")
  (tset M :todoist_resources_types [:projects
                                    :items
                                    :notes
                                    :labels
                                    :sections
  ...
  (tset M :todoist_commands
        {:project_add (fn [name temp_id uuid parent child_order is_favorite]
  ...
  (tset M :create_project_sync_request
        (fn [opts]
          {:url M.url
           ...
           :data {:sync_token (or opts.request.sync_token "*")
                  :resource_types (_G.vim.json.encode [:projects
                                                       ...
                                                       :sections])}
           :timeout 100000}))
  (tset M :process_response
        (fn [response]
          (assert (= response.status 200)
                  (. M.response_status_codes response.status))
